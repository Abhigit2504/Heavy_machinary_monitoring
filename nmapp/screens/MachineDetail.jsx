...
const GfridScroller = ({ currentGfrid, fromDate, toDate, range }) => {
  ...
  return (
    <View style={styles.gfridScrollerWrapper}>
      <Text style={styles.gfridScrollerLabel}>Available GFRID's:</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        {gfrids.map(({ gfrid }, index) => (
          <Animatable.View animation="bounceIn" duration={800} delay={index * 100} key={gfrid}>
            <TouchableOpacity
              onPress={() => goToMachine(gfrid)}
              style={[styles.gfridBox, gfrid === currentGfrid && styles.gfridBoxActive]}
              activeOpacity={0.8}
            >
              <Text style={[styles.gfridText, gfrid === currentGfrid && styles.gfridTextActive]}>
                {gfrid}
              </Text>
            </TouchableOpacity>
          </Animatable.View>
